...
def mini_imagenet_folders(trainval=False):
    train_folder = '../datas/miniImagenet/train_original_224'
    val_folder = '../datas/miniImagenet/val_original_224'
    test_folder = '../datas/miniImagenet/test_original_224'
    ...
    if trainval:
        metatrain_folders = metatrain_folders + metaval_folders
...
def get_mini_imagenet_data_loader(task, num_per_class=1, split='train',shuffle = False,train=True):
    normalize = transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])

    if split == 'train':
        if train:
            dataset = MiniImagenet(task, split=split, transform=transforms.Compose([# transforms.RandomHorizontalFlip(),
